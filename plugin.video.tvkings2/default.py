...
#MLB_Backup_2='%s:%s/enigma2.php?username=%s&password=%s&type=get_live_streams&cat_id=1211'%(host,port,username,password)
...
ABC_LOCALS='%s:%s/enigma2.php?username=%s&password=%s&type=get_live_streams&cat_id=1452'%(host,port,username,password)
CBS_LOCALS='%s:%s/enigma2.php?username=%s&password=%s&type=get_live_streams&cat_id=1453'%(host,port,username,password)
FOX_LOCALS='%s:%s/enigma2.php?username=%s&password=%s&type=get_live_streams&cat_id=1454'%(host,port,username,password)
NBC_LOCALS='%s:%s/enigma2.php?username=%s&password=%s&type=get_live_streams&cat_id=1455'%(host,port,username,password)
...
Formula_1='%s:%s/enigma2.php?username=%s&password=%s&type=get_live_streams&cat_id=1427'%(host,port,username,password)
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line3 = "To unlock your TV PRO go to:[COLOR white] http://buytvkings.mancavekodi.com[/COLOR]" 
			xbmcgui.Dialog().ok('[COLOR darkgrey]TV Kings[/COLOR]', line1, line2, line3)
			start()
		else:
			line1 = "[COLOR lime]Login Successfull![/COLOR]"
			line2 = "Welcome to [COLOR darkgrey]TV Kings[/COLOR]" 
		...
		auth = tools.OPEN_URL(auth)
		if not auth=="":
			tools.addDir('[B][COLOR white]=-=-=-=-=-=-=-=-=-=[/COLOR][/B] [B][COLOR darkgrey]TV Kings[/COLOR][/B] [B][COLOR white]=-=-=-=-=-=-=-=-=-=[/COLOR][/B]','','',icon,fanart,'')
			...
			tools.addDir('[COLOR white]Music Channels[/COLOR]',Music_Choice,25,music,fanart,'')
...
def US():
	...
	tools.addDir('[COLOR white]FOX Locals[/COLOR]',USA_Music,25,networks,fanart,'')
	tools.addDir('[COLOR white]NBC Locals[/COLOR]',USA_Music,25,networks,fanart,'')
	#tools.addDir('[COLOR white]US Networks[/COLOR]',USA_LOCAL_NETWORKS,25,networks,fanart,'')
	tools.addDir('[COLOR white]US Music[/COLOR]',USA_Music,25,music2,fanart,'')
	...
	tools.addDir('[COLOR white]NFL[/COLOR]',NFL_Package,25,nfl,fanart,'')
	tools.addDir('[COLOR white]MLB[/COLOR]',MLB_Package,25,mlb,fanart,'')
	tools.addDir('[COLOR white]MLB Extra[/COLOR]',MLB_Backup_1,25,mlb,fanart,'')
...
def UK():
	...
	tools.addDir('[COLOR white]Formula 1[/COLOR]',Formula_1,25,sports2,fanart,'')
	tools.addDir('[B][COLOR black]=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=[/COLOR][/B]','','',icon,fanart,'')
	tools.addDir('[COLOR white]Clear Cache[/COLOR]','CC',10,cache,fanart,'')
	tools.addDir('[COLOR white]Search[/COLOR]','url',5,search2,fanart,'')
	setView()
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            dialog = xbmcgui.Dialog()
            dialog.ok("[COLOR darkgrey]TV Kings[/COLOR]", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search [COLOR darkgrey]TV Kings[/COLOR]:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('[COLOR darkgrey]TV Kings[/COLOR]', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('[COLOR darkgrey]TV Kings[/COLOR]', 'Advanced Settings Removed')
	elif url =="ADS2":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('[COLOR darkgrey]TV Kings[/COLOR]', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def accountinfo():
	...
	tools.addDir('[COLOR white]=-=-=-=-=-=-=-=[COLOR darkgrey]TV Kings[/COLOR] [COLOR white]Account Info[/COLOR][COLOR white]=-=-=-=-=-=-=-=[/COLOR]','','',icon,fanart,'')
	...
	tools.addDir('To purchase account subscribe at:[COLOR white] http://buytvkings.mancavekodi.com[/COLOR]',All,2,icon,fanart,'')
	plugintools.set_view( plugintools.LIST )
...
def ivueint():
	ivuesetup.iVueInt()
	xbmcgui.Dialog().ok('[COLOR darkgrey]TV Kings[/COLOR]', 'iVue Integration Complete')
	xbmc.executebuiltin('ActivateWindow(10025,"plugin://plugin.video.IVUEcreator/update_addon/plugin.video.tvkings2",return)')
	xbmc.executebuiltin("XBMC.ActivateWindow(Home)")
...
def SIMPLEtvguidesetup():
	dialog = xbmcgui.Dialog().yesno('[COLOR darkgrey]TV Kings[/COLOR]','Would you like us to setup Simple PVR Guide for you?')
	if dialog:
		pvrsetup()
	else:
		home()
...
def IVUEtvguidesetup():
	dialog = xbmcgui.Dialog().yesno('[COLOR darkgrey]TV Kings[/COLOR]','Would You like us to Setup iVue TV Guide for You?')
	if dialog:
		ivueint()
	else:
		home()
...
def drx_settings():
	xbmc.executebuiltin("Addon.OpenSettings(plugin.video.tvkings2)")
...
def adult_settings():
	dialog = xbmcgui.Dialog().yesno('[COLOR darkgrey]TV Kings[/COLOR]','Would you like to HIDE [COLOR pink]Adult Menu[/COLOR]?', 'You can always change this in settings later on.')
	if dialog:
		...
		pass
	dialog = xbmcgui.Dialog().yesno('[COLOR darkgrey]TV Kings[/COLOR]','Would you like to PASSWORD PROTECT [COLOR pink]Adult Channels[/COLOR]?', 'You can always change this in settings later on.')
	if dialog:
...
def testarea():
	tvstreamscat   =  xbmc.translatePath(os.path.join('special://home/userdata/addon_data/plugin.video.tvkings2/categories.db'))
	channels = []
